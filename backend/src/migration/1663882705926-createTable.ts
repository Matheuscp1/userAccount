...
export class createTable1663882705926 implements MigrationInterface {
    name = 'createTable1663882705926'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`user_account\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`modifiedDate\` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`active\` tinyint NOT NULL DEFAULT 1, \`cpf\` varchar(14) NOT NULL, \`email\` varchar(100) NOT NULL, \`name\` varchar(100) NOT NULL, \`userName\` varchar(100) NOT NULL, \`password\` varchar(60) NOT NULL, UNIQUE INDEX \`index_name\` (\`cpf\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`index_name\` ON \`user_account\``);
